...
public:
    ...
    void TestBody() override
    {
        ...
        ArmSystem<TSysTraits> specimen(opts, nullptr);
...
//! @brief A test fixture class for exception vector tests. Assembles code at
//! address 0x0000 (ROM) and stays in SVC26 mode.
//! @tparam TSysTraits The data type of the traits describing the emulated
//! system to test.
template<typename TSysTraits>
...
public:
    explicit ExceptionTestFixture(const CoreTestParams &testPoint) :
        _testPoint(testPoint)
    {
    }
    ...
    void TestBody() override
    {
        ...
        ASSERT_TRUE(prepareExceptionTestSystem(&specimen, _testPoint.Code));
        ASSERT_TRUE(applyConstraints(specimen, _testPoint.Location,
                                     _testPoint.Inputs));
...
//! @brief Registers an array of ExceptionTestFixture-based unit tests.
//! @tparam TSysTraits The data type of the traits describing the emulated
//! system to instantiate in tests.
//! @param[in] sysName The name of the group of tests being registered.
//! @param[in] testPoints The array of test parameters describing the tests to
...
//! @param[in] count The count of elements in the testPoints array.
template<typename TSysTraits>
void RegisterExceptionTests(const char *sysName, const CoreTestParams *testPoints, size_t count)
{
    struct Factory
    {
    private:
    ...
    public:
        ...
        ExceptionTestFixture<TSysTraits> *operator()() const
        {
            return new ExceptionTestFixture<TSysTraits>(_testPoint);
        }
    };
